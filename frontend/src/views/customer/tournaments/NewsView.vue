...
const categories = computed(() => [
  ...
  { label: t('news.typeAnnouncement'), value: 'announcement' },
  { label: t('news.highlight'), value: 'highlight' },
  { label: t('news.analysis'), value: 'analysis' },
  { label: t('news.interview'), value: 'interview' }
])
        ...
        <div class="nav-brand">
          ...
          <el-dropdown trigger="click">
            <span class="el-dropdown-link">
              {{ selectedCategory === 'all' ? t('news.news') : getCategoryLabel(selectedCategory) }} 
        ...
        <aside class="sidebar-widgets">
          ...
          <div class="atp-widget rankings-widget">
            <div class="widget-header">
              ...
              <router-link to="/rankings" class="view-all">{{ t('news.viewAll') }}</router-link>
            </div>
            ...
            <div class="widget-body ranking-list">
              ...
              <div v-if="topPlayers.length === 0" class="empty-state">{{ t('news.updating') }}</div>
            </div>
          </div>

          <div class="atp-widget newsletter-widget">
            <h3>{{ t('news.newsletters') }}</h3>
            <p>{{ t('news.newsletterDesc') }}</p>
            <div class="input-group">
              <input type="email" :placeholder="t('news.emailPlaceholder')" />
...
.atp-news-page { background: #f1f5f9; min-height: 100vh; padding-bottom: 5rem; padding-top: 1.5rem; font-family: 'Inter', Arial, sans-serif; color: #0f172a; }
.container { max-width: 1280px; margin: 0 auto; padding: 0 1.5rem; }
.news-inner-nav { display: flex; justify-content: space-between; align-items: center; padding: 1.5rem 0; border-bottom: 2px solid #e2e8f0; margin-bottom: 2.5rem; }
.nav-brand { display: flex; align-items: center; font-weight: 800; font-size: 1.4rem; text-transform: uppercase; }
.l-label { color: #002855; }
.separator { margin: 0 12px; color: #cbd5e1; }
.el-dropdown-link { cursor: pointer; color: #0f172a; display: flex; align-items: center; font-size: 1.4rem; font-weight: 800; outline: none; }
.minimal-search { width: 300px; }
:deep(.minimal-search .el-input__wrapper) { box-shadow: none; border-bottom: 2px solid #cbd5e1; border-radius: 0; padding: 0; background: transparent; }
:deep(.minimal-search .el-input__wrapper.is-focus) { border-bottom-color: #002855; }
.magazine-layout { display: grid; grid-template-columns: 2fr 1fr; gap: 2.5rem; }
.news-main-feed { display: flex; flex-direction: column; gap: 2rem; }
.featured-card { background: white; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; cursor: pointer; box-shadow: 0 4px 15px rgba(0,0,0,0.03); transition: transform 0.3s, box-shadow 0.3s; }
.featured-card:hover { transform: translateY(-4px); box-shadow: 0 10px 25px rgba(0,0,0,0.08); border-color: #002855; }
.featured-visual { position: relative; width: 100%; height: 420px; margin: 0; background: #000; }
.featured-visual img, .featured-visual video { width: 100%; height: 100%; object-fit: cover; }
.play-icon-large { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 4rem; color: white; background: rgba(0,0,0,0.5); border-radius: 50%; display: flex; align-items: center; justify-content: center; }
.featured-content { padding: 2rem; }
.category-badge { display: inline-block; background: #c1ff72; color: #002855; font-size: 0.75rem; font-weight: 800; padding: 0.3rem 0.8rem; text-transform: uppercase; margin-bottom: 1rem; }
...
.grid-thumb { position: relative; height: 200px; background: #000; }
...
.widget-header h3 { font-size: 1.1rem; font-style: italic; font-weight: 800; color: #002855; margin: 0; display: flex; align-items: center; gap: 6px; }
...
.newsletter-widget { background: #002855; color: white; padding: 2rem 1.5rem; text-align: center; }
