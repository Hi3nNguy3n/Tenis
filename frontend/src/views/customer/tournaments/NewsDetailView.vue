...
import { playerService } from '../../../services/playerService' // Thêm API BXH
...
const fetchData = async () => {
  loading.value = true
  try {
    // 1. Tải bài viết hiện tại
    const postData = await newsService.getPost(route.params.slug)
    post.value = postData
    window.scrollTo(0, 0)

    // 2. Tải Tin tức liên quan & Bảng xếp hạng (Chạy ngầm)
    Promise.all([
...
const getCategoryLabel = (val) => {
  const map = {
    'announcement': t('news.typeAnnouncement') || 'Thông báo',
    'highlight': t('news.highlight') || 'Highlight',
    'analysis': t('news.analysis') || 'Phân tích',
    'interview': t('news.interview') || 'Phỏng vấn'
  }
  return map[val] || (t('nav.news') || 'Tin tức')
}
</script>
...
<template>
  <div class="atp-article-page" v-loading="loading">
    ...
    <div v-if="post">
      ...
      <!-- ============================================== -->
      <!-- CHẾ ĐỘ 1: NẾU LÀ VIDEO (HIỂN THỊ MEDIA PLAYER) -->
      <!-- ============================================== -->
      ...
      <!-- ============================================== -->
      <!-- CHẾ ĐỘ 2: NẾU LÀ HÌNH ẢNH (HIỂN THỊ CINEMATIC) -->
      <!-- ============================================== -->
      ...
      <div class="container main-container" :class="{ 'is-video-layout': isVideo(post.thumbnail_url || post.media_url) }">
          ...
          <div class="article-meta-modern">
             <div class="author-block">
               ...
               <div class="author-info">
                 <strong>{{ t('news.admin') || 'Saigon Tennis Tour' }}</strong>
                 <span>{{ t('news.editorial') || 'Ban biên tập' }}</span>
               </div>
             </div>
        ...
        <!-- LAYOUT 2 CỘT (NỘI DUNG VÀ SIDEBAR) -->
        <div class="article-layout">
          
          <!-- Cột Social (Trái) -->
          <aside class="article-social">
            <div class="social-sticky">
              <el-button circle :icon="Share" size="large" plain title="Chia sẻ" />
              <el-button circle :icon="ChatLineRound" size="large" plain style="margin-top: 15px;" title="Bình luận" />
            ...
            <footer class="article-footer-tags">
              ...
              <div class="action-footer">
                <el-button type="primary" size="large" class="back-btn" @click="router.push('/news')">
                  <el-icon><ArrowLeft /></el-icon> {{ t('news.backToNews') || 'Quay lại Tin tức' }}
                </el-button>
              </div>
            </footer>
          </main>
          ...
          <aside class="article-sidebar">
            ...
            <div class="atp-widget rankings-widget" v-if="topPlayers.length > 0">
              ...
              <div class="widget-footer">
                 <router-link to="/rankings" class="view-all">View Full Rankings ➔</router-link>
              </div>
            </div>
...
.progress-fill { 
  height: 100%; background: #c1ff72; /* Xanh tennis */
  transition: width 0.1s linear; 
}

/* =========================================================
   CHẾ ĐỘ VIDEO (THEATER MODE)
========================================================= */
.video-theater-mode {
  background: #000000;
  padding: 3rem 0 4rem;
...
.video-player-wrapper {
  max-width: 1000px; /* Khung video tiêu chuẩn */
  margin: 0 auto;
  aspect-ratio: 16 / 9;
  background: #0f172a;
  border-radius: 12px;
...
.main-video {
  width: 100%;
  height: 100%;
  object-fit: contain; /* Không cắt xén video */
  display: block;
}

/* =========================================================
   CHẾ ĐỘ HÌNH ẢNH (CINEMATIC)
========================================================= */
...
.hero-overlay {
  ...
  width: 100%; height: 60%;
  background: linear-gradient(to top, #f1f5f9 0%, rgba(241, 245, 249, 0) 100%); 
}
...
.main-container {
  max-width: 1280px;
  margin: -100px auto 0; /* Kéo đè lên hình ảnh */
  position: relative;
  z-index: 10;
  padding: 0 1.5rem;
}

/* Nếu là giao diện có Video ở trên, hủy margin âm đi */
.main-container.is-video-layout {
  margin-top: 2rem;
}
...
.category-badge {
  display: inline-block; background: #002855; color: white;
...
.article-header-modern h1 { 
  ...
  line-height: 1.2; margin: 0 0 2rem 0; letter-spacing: -0.02em;
}
...
.scroll-label {
  margin-top: 30px; font-size: 11px; font-weight: 700; letter-spacing: 3px;
  writing-mode: vertical-rl; color: #94a3b8;
}
...
.article-lead {
  ...
  color: #1e293b; margin-bottom: 2.5rem; font-style: italic;
  padding-bottom: 2rem; border-bottom: 2px solid #002855;
}
...
.article-content-rich :deep(img), .article-content-rich :deep(video) { 
  width: 100%; border-radius: 8px; margin: 2.5rem 0; box-shadow: 0 4px 10px rgba(0,0,0,0.1); 
}
.article-content-rich :deep(blockquote) { 
  border-left: 6px solid #c1ff72; background: #002855; 
  padding: 2rem; margin: 3rem 0; font-style: italic; 
  font-size: 1.3rem; color: white; border-radius: 0 8px 8px 0; 
}
...
.widget-header {
  padding: 1.2rem 1.5rem; border-bottom: 2px solid #002855;
  background: #f8fafc;
}
.widget-header h3 {
  font-size: 1rem; font-weight: 800; color: #002855; margin: 0;
...
@media (max-width: 1200px) {
  .article-layout { grid-template-columns: 60px 1fr; }
...
@media (max-width: 900px) {
  ...
  .article-header-modern { padding: 2rem; margin: -50px auto 2rem; }
  .main-container.is-video-layout .article-header-modern { margin-top: 0; }
  .article-header-modern h1 { font-size: 2.2rem; }
  .article-main { padding: 2rem; }
  .video-theater-mode { padding: 1.5rem 0; }
}

@media (max-width: 600px) {
  .article-hero-cinematic { height: 40vh; min-height: 300px; }
  .article-header-modern { padding: 1.5rem; margin-top: -30px; border-radius: 8px;}
  .article-header-modern h1 { font-size: 1.8rem; margin-bottom: 1.5rem; }
  .article-meta-modern { flex-direction: column; align-items: flex-start; gap: 1rem; }
  .article-main { padding: 1.5rem; border-radius: 8px;}
  .article-lead { font-size: 1.15rem; }
  .article-content-rich { font-size: 1.1rem; }
  .article-content-rich :deep(blockquote) { padding: 1.5rem; font-size: 1.1rem; }
}
</style>